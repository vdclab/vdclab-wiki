...
#### Gene Neighborhood (GN) Analysis
 > Depending on the explicit goals, explore neigbhorhhods to find associations or create figures for a manuscript, the choice of the tool might be different. Many integrated database such as String, IMG-JGI or BV-BRC have GN tools.  They can be useful for exploration but not for figures.  The most extensive fpr exploration is the EFI/GNN.In the last years a flurry of userfrienfly tools have been published and should be explored. See  HERE for list of tools. Ones we use often are listed below.
{.is-info}
