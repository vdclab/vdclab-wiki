...
#### Gene Neighborhood (GN) Analysis
 > Depending on the explicit goals, explore neigbhorhhods to find associations or create figures for a manuscript, the choice of the tool might be different. Many integrated database such as String, IMG-JGI or BV-BRC have GN tools.  They can be useful for exploration but not for figures.  The most extensive fpr exploration is the [EFI/GNT](https://efi.igb.illinois.edu/efi-gnt/).In the last years a flurry of user frienfly tools have been published and should be explored.  Ones we use often are listed below.
{.is-info}
