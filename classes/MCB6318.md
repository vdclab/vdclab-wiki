...
> Databases that compile patwhay information and place predicted proteins in specific pathways starting with genomic information can be used to perform metabolic reconstruction, check if a pathway is complete and compare pathways between organisms. KEGG has both Pathways and Modules even if the latter have been also intergradted in the former. Metacyc/BioCyc uses now a subscription model. 
