...
> In the last 20 years different types of "omics" data  (transcriptomics, proteomics, metabolomics to name the most common) can be generated and the amount is increasing at exponential rate as the costs go down and the technology improves. 
In parallel predictions of regulatory network based on motif analyses has improved and when combined with transcriptomic data can help identify genes  that are co-regulated. One problem with all this data sets is that they are noisy and require replicates ( from 3 to 6 depdending on  the techniques).  Also Id mapping issues  make  the use of web-based analyses tools  sometimes fustrating. Many of the best tools to analyses multi-omics data are R packages but there are few web-based altarnatives to both analyze your own and published data. See addional reading Here
{.is-info}
